...
def export_video_in_folder(video_path):
    global using_yuv
    fps = 25  # 帧率
    ...
    total_frame = get_total_frames(video_path)
    ...
    for frame_index in range(total_frames):
        ...
        with zipfile.ZipFile(path_part_file, 'r') as zip_ref:
            ...
            if preview_begin_convert and frame_index == 0:
                cv2.imshow("Image", img)
                key = cv2.waitKey(0)
                cv2.destroyAllWindows()
            ...
            progress_percentage = "{:.2f}%".format((frame_index / float(total_frame)) * 100)

            print("\r将{0}转换为mp4文件，进度: {1}, 第[{2}/{3}]帧, 耗时：{4} s".format(file_name, progress_percentage, frame_index,
                                                                         total_frame,
                                                                         round(time_consuming, 2)), end="")
