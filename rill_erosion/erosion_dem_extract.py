...
def create_surface(xyz):
    ...
    grid_x, grid_y = np.mgrid[x_min:x_max:200j, y_min:y_max:800j]  # 200x800 grid
...
def sample_min_elevation_change(dem_diff, step=5):
    """
    沿y轴采样最低高程变化
    :param dem_diff: 高程差异矩阵
    :param step: y轴采样步长
    :return: 采样后的y轴坐标和对应的最低高程变化
    """
    y_coords = np.arange(0, dem_diff.shape[1], step)
    min_elevation_change = []
    for y in y_coords:
        # 对每个步长区间找到最低高程变化
        min_change = np.min(dem_diff[:, y:y+step])
        min_elevation_change.append(min_change)
    return y_coords, min_elevation_change
